for i in `ls *_1.fq.gz`; do base=$(basename $i "_1.fq.gz"); nohup bowtie2 -x ./genomes/mm10_bowtie2/mm10 -1 ${base}_1.fq.gz -2 ${base}_2.fq.gz --dovetail --very-sensitive-local --no-unal --no-mixed --no-discordant -X 2000 -S ${base}.mm10.sam 2> ${base}.bowtie2.align.log -p10; done
...
for i in `ls *_unique.bam`; do base=$(basename $i "_unique.bam");java -jar ./picard-tools-2.2.4/picard.jar MarkDuplicates REMOVE_DUPLICATES=true I=$i o=${base}_rmdup.bam M=${base}_picard.txt;done
