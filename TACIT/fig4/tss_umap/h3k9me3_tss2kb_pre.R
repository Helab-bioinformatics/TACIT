...
for (i in 1:length(stage)) {
  poster <- read.csv(paste0(stage[i], "_heterochromatin_h3k9me3_tss2kb_posterior.txt"), sep = "\t")
  mat[[i]] <- apply(poster,1,mean)
}
all_mat <- do.call(rbind,mat)
write.table(mat, "all_heterochromatin_h3k9me3_tss2kb_posterior.txt", sep = "\t", quote = F)
