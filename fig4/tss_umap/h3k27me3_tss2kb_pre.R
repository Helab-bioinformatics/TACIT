...
############################## heterochromatin_h3k27me3  ################################
rm(list = ls())
stage <- c("zygote",  paste0("2cell_", 1:2), paste0("4cell_", 1:4), paste0("8cell_", 1:8))
mat <- list()
for (i in 1:length(stage)) {
  poster <- read.csv(paste0(stage[i], "_heterochromatin_h3k27me3_tss2kb_posterior.txt"), sep = "\t")
  mat[[i]] <- apply(poster,1,mean)
}
all_mat <- do.call(rbind,mat)
write.table(mat, "all_heterochromatin_h3k27me3_tss2kb_posterior.txt", sep = "\t", quote = F)
